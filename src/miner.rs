...
impl Miner {
    pub fn new(cfg: Cfg) -> Miner {
		...
		for plot_dir_str in &cfg.plot_dirs {
			...
			info!("path={}, files={}, size={} TiB",plot_dir_str,num_plots,local_capacity/4/1024/1024);
			global_capacity += local_capacity;
            if num_plots == 0 {
                warn!("no plots in {}", plot_dir_str);
            }
        }
		info!("plot files loaded, total capacity={} TiB",global_capacity/4/1024/1024);
            ...
            self.rx_nonce_data
                .for_each(move |nonce_data| {
                    ...
                    if state.best_deadline > deadline && deadline < target_deadline {
                        ...
                        info!("deadline found, nonce={} -> deadline={}", nonce_data.nonce, deadline);
                    }
